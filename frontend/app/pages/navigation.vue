<template>
  <div class="min-h-[calc(100vh-64px-80px)] py-16 px-4 bg-white">
    <UContainer class="max-w-7xl mx-auto space-y-8">
      ...
      <!-- Image + infos -->
      <div v-if="images.length > 0" class="flex flex-col lg:flex-row gap-10 items-start justify-center">
        <!-- Image -->
        <div class="flex-1 text-center">
          ...
          <div class="flex justify-between mt-4">
            <UButton @click="precedente" :disabled="index === 0" color="gray" variant="ghost">◀ Précédente</UButton>
            <UButton @click="suivante" :disabled="index === images.length - 1" color="gray" variant="ghost">Suivante ▶</UButton>
          </div>
        </div>

        <!-- Fiche image -->
        <div class="flex-1 space-y-2 bg-[#f8f9fa] p-6 rounded-xl shadow-sm border text-sm text-[#1b263b]">
          <h2 class="text-lg font-semibold mb-2">Informations de l'image</h2>
          <p><strong>Taille :</strong> {{ meta.Size }} ko</p>
          <p><strong>Dimensions :</strong> {{ meta.Width }} x {{ meta.Height }}</p>
          ...
          <div class="w-full max-w-sm mx-auto bg-white p-4 rounded-xl shadow-sm border text-center text-sm text-[#1b263b]">
              ...
              <div class="w-60 h-60">
                <PieChart
                  :data="{
                    ...
                    datasets: [{
                      label: 'Moyenne RGB',
                      data: [meta.Avg_R, meta.Avg_V, meta.Avg_B],
          ...
          <p><strong>Statut :</strong> {{ meta.Status === true ? 'Pleine' : 'Vide' }}</p>
          <!-- Carte de localisation -->
          <div class="w-full max-w-sm mx-auto mt-6">
            <h3 class="text-base font-semibold text-[#1b263b] mb-2 text-center">Localisation</h3>
            <MapMarker :lat="location.lat" :lng="location.lng" :city="location.city" />
          </div>
        </div>
      </div>
...
async function fetchImageData(filename) {
  try {
    const res = await axios.get(`http://localhost:8000/api/image/${filename}`)
    meta.value = res.data
  } catch (err) {
